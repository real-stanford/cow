...
class ClipActorCritic(ActorCriticModel[CategoricalDistr]):
    ...
    def __init__(
        ...
        if device is None:
            ...
            model, _ = clip.load(clip_type, device=device)
        ...
        # NOTE: might want to remove last pooling layers or something

        self.rgb_encoder = model.visual
        ...
        if use_depth:
            ...
            self.late_fusion = Linear(
                CLIP_MODEL_TO_FEATURE_DIM[clip_type]*2, CLIP_MODEL_TO_FEATURE_DIM[clip_type])
    ...
    def forward(
        ...
        rgb_observation = None
        if next(self.rgb_encoder.parameters()).is_cuda:
            rgb_observation = observations['rgb'].half()
        else:
            rgb_observation = observations['rgb'].float()

        rgb_observation_encoding = compute_cnn_output(
            self.rgb_encoder, rgb_observation)

        observation_encoding = rgb_observation_encoding / \
            rgb_observation_encoding.norm(dim=-1, keepdim=True)

        if self.use_depth:
            # NOTE: this might not be a good idea lol
            # simple linear layer to do late fusion between CLIP RGB and Depth features
            ...
            observation_encoding = self.late_fusion(
                torch.cat((observation_encoding.float(), ddd_observation_encoding.float()), dim=-1))
            observation_encoding = observation_encoding / \
                observation_encoding.norm(dim=-1, keepdim=True)

        x_out, rnn_hidden_states = self.state_encoder(
            observation_encoding.float(), memory.tensor("rnn"), masks
        )
